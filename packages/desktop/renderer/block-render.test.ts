...
describe("freezing a block's output", () => {
  it("renders plain text as lines of spans", () => {
  ...
  it("paints an ANSI colour with the theme's own value", () => {
  ...
  it("carries bold, italic and underline across", () => {
  ...
  it("renders markup in program output as text, never as elements", () => {
    const element = renderOutput("<img src=x onerror=alert(1)>\r\n", 80);
    expect(element.querySelector("img")).toBeNull();
    expect(element.textContent).toContain("<img src=x onerror=alert(1)>");
  });

  it("keeps wide characters in one cell group", () => {
