 ...
 *  knows about. */
export function createRealDockerClient(
  env: EnvSource = process.env,
  platform?: NodeJS.Platform,
): DockerClient {
  return createDockerClient(
    (command, args) => runCommand(command, args, resolveEnv(env), platform),
    (command, args, onChunk) => {
      ...
      const target = spawnTarget(command, args, resolveEnv(env), platform ?? "linux");
