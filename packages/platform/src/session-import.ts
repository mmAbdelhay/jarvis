...
export function summaryOf(text: string): string | undefined {
  if (isLocalCommandNoise(text)) return undefined;
  return trimSummary(readableCommand(text));
}

/**
 * Whether a user turn is the CLI talking to itself rather than the user.
 *
 * Running a slash command writes two extra "user" records: a caveat telling
 * the model to ignore what follows, and the command's own stdout. Neither
 * was typed by anyone. Taken as a session's summary they bury the real
 * prompt — 10 of 96 rows read "Caveat: The messages below were generated by
 * the user while running local commands..." before this existed — so such a
 * turn is skipped and the search continues to the next one.
 */
function isLocalCommandNoise(text: string): boolean {
  return /^\s*<local-command-(?:caveat|stdout|stderr)>/.test(text);
}
